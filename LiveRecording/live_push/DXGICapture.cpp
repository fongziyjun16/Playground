...
bool DXGICapture::CaptureRgb32(unsigned char* rgbBuffer, const int rgbBufferSize)
{
    ...
    HRESULT hr = m_pDXGIOutputDuplication->AcquireNextFrame(0, &frameInfo, &pDesktopResource);
    if (FAILED(hr)) {
    ...
    /*if (m_pCopyBuffer == nullptr) {*/
        ...
        m_pDX11Texture->GetDesc(&copyBufferDesc);
    ...
    hr = m_pDX11DeviceContext->Map(m_pCopyBuffer, 0, D3D11_MAP_READ, 0, &mappedResource);
    if (SUCCEEDED(hr)) {
        memcpy(rgbBuffer, mappedResource.pData, rgbBufferSize);
        m_pDX11DeviceContext->Unmap(m_pCopyBuffer, 0);
        RELEASE_OBJECT(m_pCopyBuffer);
    }*/
