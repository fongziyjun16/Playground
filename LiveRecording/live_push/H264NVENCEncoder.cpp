...
bool H264NVENCEncoder::IsSupported(int width, int height)
{
	// av_register_all(); //注册和编解码器有关的组件
	const AVCodec* pH264Codec = avcodec_find_encoder_by_name(encoder_name); // 利用编码器名称查找编码器(AVCodec.name)
	...
	pEncoderContext->time_base = {1, 60};
	pEncoderContext->pix_fmt = *pH264Codec->pix_fmts;
	pEncoderContext->width = width;
	pEncoderContext->height = height;

	bool ret = avcodec_open2(pEncoderContext, pH264Codec, nullptr) >= 0;
...
bool H264NVENCEncoder::OpenEncoder(int width, int height, int frameRate, int bitrate)
{
	// av_register_all();
	const AVCodec* pH264Codec = avcodec_find_encoder_by_name(encoder_name); // 利用编码器名称查找编码器(AVCodec.name)
	if (nullptr == pH264Codec) {
...
void H264NVENCEncoder::CloseEncoder()
{
...
int H264NVENCEncoder::Encoder(const unsigned char* inImageData, unsigned char* outH264Buffer, int outH264BufferSize)
{
	...
	AVPacket* outAVPacket = av_packet_alloc();
	outAVPacket->data = NULL;
	outAVPacket->size = 0;

	int ret = avcodec_send_frame(m_pEncoderContext, m_pAVFrame);
	if (ret < 0) {
		// Handle error
		av_packet_free(&outAVPacket);
		return 0;
	}
	
	// int gotPacket = 0;
	// flg = avcodec_encode_video2(m_pEncoderContext, &outAVPacket, m_pAVFrame, &gotPacket);
	flg = avcodec_receive_packet(m_pEncoderContext, outAVPacket);
	// if (flg == 0 && gotPacket != 0 && outAVPacket.size < outH264BufferSize)
	if (flg == 0 && outAVPacket->size < outH264BufferSize)
	{
		int size = 0;
